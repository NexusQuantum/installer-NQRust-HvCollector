...
pub struct App {
    ...
    pub(crate) kubeconfig_exists: bool,
    ...
    /// True when running as airgapped binary (offline mode, no image pull)
...
impl App {
    pub fn new() -> Self {
        ...
        let kubeconfig_exists = utils::find_file("kubeconfig.yaml");
    ...
    async fn fetch_github_username(&self, token: &str) -> Result<String> {
        ...
        let response = client
            ...
            .header("User-Agent", "nqrust-hvcollector")
    ...
    async fn self_update(
        ...
        let deb_path = env::temp_dir().join(format!("nqrust-hvcollector-{}.deb", version_label));
        ...
        if let Some(sum_url) = checksum_url {
            ...
            let sums_path = env::temp_dir().join("nqrust-hvcollector-SHA256SUMS");
